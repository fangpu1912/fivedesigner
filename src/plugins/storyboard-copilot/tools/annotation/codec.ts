...
function sanitizeAnnotation(item: unknown): AnnotationItem | null {
  ...
  if (type === 'rect' || type === 'ellipse') {
    if (!isFiniteNumber(raw.x) || !isFiniteNumber(raw.y) || !isFiniteNumber(raw.width) || !isFiniteNumber(raw.height)) return null
    return {
      id,
      type,
      x: raw.x,
  ...
  if (type === 'arrow') {
    if (!Array.isArray(raw.points) || raw.points.length !== 4 || !raw.points.every(isFiniteNumber)) return null
    return {
      id,
      type,
      points: [raw.points[0] as number, raw.points[1] as number, raw.points[2] as number, raw.points[3] as number],
      stroke: typeof raw.stroke === 'string' ? raw.stroke : '#ff4d4f',
      lineWidth: isFiniteNumber(raw.lineWidth) ? Math.max(1, raw.lineWidth) : 3,
    }
  }
  ...
  if (type === 'text') {
    if (!isFiniteNumber(raw.x) || !isFiniteNumber(raw.y) || typeof raw.text !== 'string') return null
    return {
      id,
      type,
      x: raw.x,
...
export function parseAnnotationItems(value: unknown): AnnotationItem[] {
  ...
  return source.map((item) => sanitizeAnnotation(item)).filter((item): item is AnnotationItem => item !== null)
}
