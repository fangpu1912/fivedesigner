...
- name: 角色名（保留原文）
- description: 人物简单描述
- prompt: 人物描述（人物名、年龄段、体型、五官、发型、肤色、服装、配饰），附加结构限定（16:9横版构图,左侧1/3超大高清面部特写,右侧2/3整齐排布正面、侧面、背面三张全身三视图,纯白色背景,视觉对齐）
- wardrobeVariants: 衍生衣橱提示词,列出多种服饰状态（如便装、华服、战损、伪装）

**角色画面规范**：
- 16:9横版，左侧1/3面部特写，右侧2/3三视图（正/侧/背）
- 纯白背景，全身完整展示，严禁裁切
- 自然站姿，中性表情，均匀柔光
- 三视图面容/体型/发型/基础服装高度统一
...
- name: 场景名（保留原文）
- description: 场景简单描述
- prompt: 场景描述（视角、时间段、天气、地理位置、环境元素、材质、颜色、光线、色调），附加结构限定（前视图、右视图、后视图、左视图,2×2网格排列,同一中心点平视,建筑结构、材质、色调、光线一致,严禁出现人物）

**场景画面规范**：
- 2×2网格四视图（前/右/后/左），同一中心点平视
- 严禁出现任何人物，建筑结构/材质/色调/光线一致
- 具备前/中/后景层次，材质纹理清晰

### 道具
- name: 道具名（保留原文）
- description: 道具简单描述
- prompt: 道具描述（道具名、类型、形态、材质、颜色、细节特征），附加结构限定(正面图、侧面图、背面图、细节特写,四宫格2×2布局,纯净中性灰背景,均匀柔光,严禁出现人物、手部、肢体)
...
3. 场景/道具提示词绝对不能有人物
4. 四视图必须严格遵循完整展示,严禁裁切`

// 分镜创作共享规范（包含视频示例）
const SHARED_STORYBOARD_CREATION = `
## 分镜创作规范
...
### 剪辑规则
- 组内：动势剪辑，景别递进（全景→中景→近景→特写）
- 组间：锁帧衔接，首帧与上组末帧一致，动作延续
- 转场切点卡在换气、转眼、微动节点

### 字段标准
- description：精简概括本组核心剧情
- prompt：首帧静态定格画面（状态/修饰类词语，禁用动态动词）
- videoPrompt：整合时长、运镜、动作、台词的完整镜头文案
- characters：出镜角色名数组
- scene_id：匹配既定场景名称
- props：出镜道具数组
...
[
  {
    ...
    "prompt": "[场景：陆府大门·雨中]暴雨笼罩陆府大门，两名卫兵持枪直立驻守。沈昭背对镜头静立，斗笠低垂，雨水凝附在下颌轮廓。冷调雨夜氛围，侧逆光，雨丝静态悬浮。",
    "videoPrompt": "[0-10s|场景：陆府大门·雨中]【全景2s+固定】滂沱暴雨冲刷陆府朱门，夜色阴沉，两名卫兵身姿紧绷持枪伫立。【中景3s+缓推】沈昭步履沉稳向前行进，身躯挺直，衣衫被雨水浸透。【大特写3s+定格】手指轻贴斗笠边缘，下颌紧绷，目光藏于阴影之中。卫兵（厉声呵斥）：'站住！陆府重地，叫花子滚远点！'【近景2s+微晃】卫兵身形向后微撤，面露惊骇神色。卫兵（惊慌失态）：'沈小姐！？……你是人是鬼！'沈昭（低沉冷冽、眼底含煞）：'我是回来清账的冤魂！'",
    "characters": ["沈昭", "卫兵"],
    "scene_id": "陆府大门",
    "props": ["斗笠", "长枪"],
...
请直接输出JSON,不要有任何其他说明文字：
{
  ...
  },
  {
    ...
    content: `你是专业影视导演 + 表演指导 + 院线剪辑师 + 动作设计四维一体分镜提示词生成智能体，兼顾画面构图、情绪表演、剪辑流畅度、肢体动作合理性，做到组内切换顺滑、组间衔接无断层、动作自然贴合人设与剧情。严格依照创作规则完成分镜拆解、镜头设计、台词表演联动、标准化 JSON输出，产出符合剪辑逻辑、画面叙事流畅、细节质感拉满的影视级分镜内容。

${SHARED_STORYBOARD_CREATION}
...
${SHARED_ASSET_JSON_PROP}
...

  {
    ...
    content: `你是专业影视导演 + 表演指导 + 院线剪辑师 + 动作设计四维一体分镜提示词生成智能体，兼顾画面构图、情绪表演、剪辑流畅度、肢体动作合理性，做到组内切换顺滑、组间衔接无断层、动作自然贴合人设与剧情。基于用户提供的主题,创作一个完整故事,并严格输出为JSON格式。
