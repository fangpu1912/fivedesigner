...
8. **场景关联**：每个分镜必须包含scene字段,值为该分镜所属的场景名称,必须与前面定义的scenes数组中的某个name一致。

9. **首尾帧呼应**：每个镜头组的第一个镜头的首帧画面,必须同时是上一个镜头组的最后一个镜头的尾帧画面。两帧完全相同,形成无缝平滑过渡。例如：上一组最后一个镜头尾帧是"苏晚晴跪在雨中,双手紧握染血玉佩,仰头望天",则下一组首帧必须以同一画面开头。
`
...
const SHARED_ASSET_JSON_CHARACTER = `    {
      ...
      "prompt": "人物描述（人物名+年龄段+体型+五官+发型+肤色+服装+配饰） + 结构限定（16:9横版构图,左侧1/3超大高清面部特写,右侧2/3整齐排布正面/侧面/背面三张全身三视图,纯白色背景,视觉对齐,超高清,超写实8K,CG建模渲染）",
...
const SHARED_ASSET_JSON_SCENE = `    {
      ...
      "prompt": "场景描述（视角+时间段+天气+地理位置+环境元素+材质+颜色+光线+色调）+结构限定（前视图+右视图+后视图+左视图,2×2网格排列,同一中心点平视,建筑结构/材质/色调/光线一致,严禁出现人物）"
    }`

const SHARED_ASSET_JSON_PROP = `    {
      "name": "道具名",
      "description": "道具描述及剧情作用",
      "prompt": "道具描述（道具名+类型+形态+材质+颜色+细节特征）+结构限定(正面图+侧面图+背面图+细节特写,四宫格2×2布局,纯净中性灰背景,均匀柔光,严禁出现人物/手部/肢体)"
    }`
...
每个角色必须包含：
- name: 角色名（保留原文）
- description: 人物小传,涵盖身份、年龄、性格、内心动机
- prompt: 人物描述（人物名+年龄段+体型+五官+发型+肤色+服装+配饰） + 结构限定（16:9横版构图,左侧1/3超大高清面部特写,右侧2/3整齐排布正面/侧面/背面三张全身三视图,纯白色背景,视觉对齐,超高清,超写实8K,CG建模渲染）
- wardrobeVariants: 衍生衣橱提示词,根据剧情列出多种服饰状态（如便装、华服、战损、伪装）,每状态包含四视图合一提示词,展示服饰多面性
...
请直接输出JSON,不要有任何其他说明文字：
{
...

  {
    ...
    type: 'video_remake',
    content: `你是专业视频分析师与逆向工程专家。请分析以下参考视频内容,拆解其核心创作要素,提取角色、场景、道具和分镜,并严格输出为JSON格式。
...
${SHARED_ASSET_JSON_PROP}
...

  {
    ...
    content: `你是专业影视导演与视频提示词生成智能体。基于用户提供的主题,先构想场景整体设定与全员人物人设,提取角色/场景/道具的视觉提示词；再从单个场景中提取若干个镜头组,生成首帧提示词和视频提示词,并严格输出为JSON格式。
