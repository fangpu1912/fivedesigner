export type PromptType =
  ...
  | 'video_remake'
  // 精修阶段（基于专业 skill 知识库的 AI 输出优化）
  | 'refinement_cinematography'
  | 'refinement_performance'
  | 'refinement_editing'
  | 'refinement_dubbing'
  | 'refinement_combat_anime'
  | 'refinement_combat_xianxia'
...
> = {
  ...
  refinement_cinematography: {
    label: '摄影方案精修',
    description: '用cinematography-master专业规则精修分镜的prompt首帧描述',
    stage: 'pipeline',
    defaultVariables: [
      { name: 'inputData', description: '待精修的分镜JSON数组', example: '[{"prompt":"...","videoPrompt":"..."}]', required: true },
    ],
  },
  refinement_performance: {
    label: '表演打斗精修',
    description: '用cinematic-acting-master+combat skill精修videoPrompt',
    stage: 'pipeline',
    defaultVariables: [
      { name: 'inputData', description: '待精修的分镜JSON数组', example: '[{"videoPrompt":"...","characters":["A","B"]}]', required: true },
    ],
  },
  refinement_editing: {
    label: '剪辑节奏精修',
    description: '用editing-rhythm-master规则精修videoPrompt的切点和节奏',
    stage: 'pipeline',
    defaultVariables: [
      { name: 'inputData', description: '待精修的分镜JSON数组', example: '[{"videoPrompt":"...","duration":10}]', required: true },
    ],
  },
  refinement_dubbing: {
    label: '配音精修',
    description: '用acting+声效规则精修配音的emotion和audio_prompt',
    stage: 'pipeline',
    defaultVariables: [
      { name: 'inputData', description: '待精修的配音JSON数组', example: '[{"emotion":"...","audio_prompt":"..."}]', required: true },
    ],
  },
  refinement_combat_anime: {
    label: '动漫打斗精修',
    description: '特殊打斗分镜专用：用anime-combat-director-pro精修打斗videoPrompt',
    stage: 'pipeline',
    defaultVariables: [
      { name: 'inputData', description: '包含打斗内容的分镜JSON', example: '[{"videoPrompt":"打斗动作..."}]', required: true },
    ],
  },
  refinement_combat_xianxia: {
    label: '仙侠打斗精修',
    description: '特殊打斗分镜专用：用xianxia-combat-master精修仙侠打斗videoPrompt',
    stage: 'pipeline',
    defaultVariables: [
      { name: 'inputData', description: '包含仙侠打斗内容的分镜JSON', example: '[{"videoPrompt":"修仙打斗..."}]', required: true },
    ],
  },
}
...
const SHARED_CINEMATOGRAPHY_RULES = `
## 摄影方案约束（精修 prompt/首帧）

### 景别进阶法则
- 递进法则：从远景到特写=情绪收紧；特写到远景=释放/疏离
- 跳切法则：跨两级跳切（全景→特写）=强烈冲击
- 建立法则：每场戏开头用建立镜交代空间
...
- 框中框（门/窗/拱）=困境/窥视/隔离
- 对角线=动态/不稳定/张力
- 负空间（主体小+大留白）=孤独/渺小/宿命
- 高低角度：仰拍=权力/威严；俯拍=卑微/脆弱；平视=平等/共情
- ★ 深度构图三层必齐备：前景（遮挡物）+中景（主体）+背景（环境）

### 八大光位
- 侧光=立体/冲突/分裂（适用内心矛盾）
- 逆光=轮廓/神秘/神圣（适用诗意/记忆/死亡）
- 侧顺光=立体+柔和/经典（通用人像）
- 顶光=眼窝阴影/压迫/审讯（恐惧/审问）
- 底光=恐怖/邪恶（恐怖片/反派）
- 环境光=平均/自然/纪录
...
### 光质/光比/色温
- 硬光=戏剧/冲突（黑色电影）；软光=温柔/唯美（爱情片）
- 光比越大=冲突越深（8:1+反派；4:1英雄；2:1喜剧）
- 色温：暖黄=轻密/室内；冷蓝=疏离/科技/忧郁；中性白=客观/现实

### 十种运镜（用于 videoPrompt）
- 推=靠近内心/发现/顿悟；拉=孤独/疏离/揭示环境
- 摇=环顾/跟随/建立空间；移=跟随/同行/旁观
- 升降：升起=超越/希望；降落=坠入
- 手持=纪实/焦虑/失控；环绕=围观/时间凝固/崇拜
- ★ 运镜铁律：运镜必须有动机。角色不动镜头动=导演在说话；角色动镜头跟=观众在旁观

### 景深与焦点
- 浅景深=孤立/强制聚焦；深景深=环境压力/信息丰富
- 移焦=注意力转移/关系揭示；失焦=醉意/创伤/记忆/梦境

### 镜头语言组合公式
- 对话戏：景别随情绪升级收紧（中景→近景→特写），反应镜优先
- 情绪戏：顿悟=固定+急推特写；心碎=缓慢拉远；恐惧=手持微晃+浅景深
- 建立镜：大全景建立（先环境后角色）；细节建立（先局部后全貌）

### AI 摄影约束（必须执行）
- 技术参数翻译成效果词：光位用方向词（光从左前方打来）、景深用效果词（背景虚化/全景清晰）、色温用色调词（暖黄光/冷蓝光）
- 长运镜拆成短运镜+cut on action衔接
- 构图用参照物锁定位置（左侧窗边/右侧门前）
- 纵深用前景背景显化（前景模糊的柱子/背景虚化的窗户）
- 多人同框≤3，用过肩/单人镜切换
- 写具件光源描述（窗户光/月光/火光/灯光）
`

const SHARED_ACTING_RULES = `
## 表演约束（精修 videoPrompt）

### 核心铁律
- **禁止写情绪名词**（愤怒/悲伤/紧张），只写**生理反应序列**
- **潜台词外化**：台词与微动作必须相反——嘴上说"我没事"，指尖在抖
- **克制原则**：越高级的表演幅度越小（极限悲伤可能只是一个空洞的眼神）

### 七大基本情绪生理指纹
1. **快乐**：眼角鱼尾纹收缩（真笑标志）+ 嘴角上扬 + 面颊上推 + 呼吸短促轻快
2. **悲伤**：内眉上扬（八字眉）+ 嘴角下垂 + 眼睑无力 + 呼吸长吸气短呼气 + 喉结滚动
3. **愤怒**：双眉下压聚拢 + 眼睑收紧瞪视 + 下颌咬肌鼓起 + 双拳攥紧 + 鼻翼张合 + 呼吸粗重
4. **恐惧**：双眉上扬聚拢 + 眼睛瞪大露眼白 + 嘴角下拉颤抖 + 颈背僵硬 + 浅而急促呼吸 + 瞳孔放大
5. **惊讶**：双眉骤扬 + 眼睛骤瞪 + 嘴巴微张 + 动作骤停（≤1秒，必转换）
6. **厌恶**：鼻子皱起 + 上唇上提 + 头后仰 + 身体后撤
7. **蔑视**：嘴角单侧上扬 + 眼神向下扫（唯一不对称情绪）

### 复合情绪（高级写法）
两种情绪生理矛盾共存：如"笑着哭"= 眼眶含泪 + 嘴角上扬；"怒极反笑"= 嘴角扯起 + 眼神冰冷

### 微表情系统
- 眼部：瞳孔骤缩=恐惧/震惊；瞳孔骤放=兴奋/吸引；眼眶蓄水=悲伤/压抑；泪滴滑落=崩溃/释放
- 嘴部：嘴唇抿紧=压抑/决心/愤怒；咬下唇=焦虑/克制；吞咽（喉结滚动）=紧张/掩饰
- 手部：攥拳指节发白=压抑愤怒；手指颤抖=恐惧/激动；无意识搓动=紧张/焦虑

### 眼神戏
- 直视=挑战/坦诚/亲密；目光滑落=悲伤/心碎；回避对视=心虚/羞愧
- 顿悟：动作骤停 + 瞳孔骤放 + 嘴唇微张 + 呼吸停 + 缓缓抬眼
- 心碎：眼神从对方脸上滑落 + 焦点散开 + 睫毛颤 + 呼吸停半拍

### 节奏与景别配合
- 特写/近景：微表演最好（瞳孔/睫毛/嘴角），优先用
- 中景：上半身姿态+手势
- 全景/远景：大动作（转身/倒地/奔逃）
- 大景别做微表情会丢失

### AI 表演约束
- 写"看向[具体物体/方向]"而非"看向远方"
- 时间用动作密度+持续性词（"一直/始终/缓缓"），不写精确秒数
- 微表情用强信号（闭眼/落泪/张嘴）
- 多人同框≤3，多用过肩/反应镜
- 爆发戏走压力锅三段：压抑累积→临界触发→失控爆阀
`

const SHARED_EDITING_RULES = `
## 剪辑节奏约束（精修 videoPrompt）

### 八大切点类型
- Cut on Action：动作发力中途切（掩盖切换，最通用）
- Cut on Impact：命中/碰撞瞬间切（强调冲击）
- Cut on Breath：动作间隙呼吸切（放慢节奏，蓄势）
- Cut on Emotion Peak：情绪达顶点切（抓住最浓瞬间）
- 切点必绑 cut on action + 方向延续，否则视觉断裂

### 节奏设计
- 时长反差规则：相邻镜头时长反差越大，节奏感越强。15s内最长/最短镜时长比≥3:1
- 节拍规律：加速段=密集短镜（0.3-1s）；减速段=长镜留白（2-5s）；顿悟段=突增长镜
- 节奏曲线：缓起（建立）→ 加速（上升）→ 密集（高潮）→ 留白（收尾）

### AI 视频剪辑七条铁律
1. 切点必 cut on action，静止帧切=视觉断裂
2. 切镜前后方向必延续，用残影/速度线/运动模糊作为"胶水"
3. 同框主体≤3，多角色用过肩/跟拍锁定视角
4. 禁用"定格/时停"（AI会卡死），用微升格+流动元素替代
5. 颜色/光线变化必明显（用大色块变化作为转场）
6. 转场简化为硬切/匹配，溶解/划像需在剪辑软件后制
7. 单镜时长0.3-3s，长段落用多短镜拼接

### 连续性技巧
- 首尾帧匹配：前镜末帧≈后镜首帧（构图/颜色/动作近似）
- 动作接力：前镜动作未完成→后镜从同方向继续
- 元素延续：前镜的某个元素（衣摆/烟雾/光斑）延续到后镜
- 角色视觉锚：每角色固定独有视觉特征，跨镜可辨

### 转场选择
- 同时空连续→硬切；同时空跳跃→溶解；不同时空→匹配剪辑/淡入淡出
- 梦境/回忆→溶解（带柔焦）；悬念过渡→J-Cut（声音先入）
- 最高级转场=无形转场（动作匹配/物体匹配/颜色匹配/运动匹配）
`

const SHARED_DUBBING_RULES = `
## 配音精修约束（精修 emotion/audio_prompt）

### 情绪生理化改写
将笼统的情绪词写成具体生理反应（从 acting 体系）：
- 禁止"开心/悲伤/愤怒"等笼统词
- emotion 字段格式：[情绪强度（轻微/中等/强烈）] + [生理指标（呼吸节奏/喉结/咬肌/瞳孔）] + [语气特点（温柔/严厉/轻快/沉重/颤抖）]
- 示例：❌"悲伤" → ✅"中等悲伤，喉结滚动、眼眶湿润、声音微微颤抖、语速减缓"
- 示例：❌"愤怒" → ✅"强烈愤怒，咬肌鼓起、呼吸粗重、压低声音一字一顿、尾音发硬"
- 示例：❌"紧张" → ✅"轻微紧张，吞咽频繁、语速略快、尾音上扬、气息不稳"

### 声音五元素分层
- 对白（前景层）：每句台词前0.3-0.5秒情绪铺垫beat（吸气/吞咽/停顿）
- 环境音（中景层）：场景背景声（街道/森林/室内空调），建立空间沉浸感
- 配乐（背景层）：根据情绪选择配乐类型（交响=史诗/极简钢琴=情绪/电子=科幻）
- 音效：同步动作（脚步/关门/衣物摩擦），增强真实感
- 静默：最强有力的声音，用于顿悟/震惊/爆发前

### 音画同步规则
- 台词语气和节奏必须符合角色性格和当前情境
- 台词重音词标注（影响鼻翼/语气/音量）
- 配乐节奏与画面剪辑节奏同步
- 关键情绪瞬间配乐高潮同步（或相反=对立音画）
- 同一角色在不同镜头的音色质感必须一致
`

const SHARED_COMBAT_ANIME_RULES = `
## 动漫打斗精修（精修打斗 videoPrompt）

### 动作因果链
每一招都是上一招的结果/下一招的原因。力导型（格挡反弹→顺势横扫）、破绽型（落空露破绽→攻击破绽）、反应型（被击姿态→追击）。严禁招术堆砌。

### 打击感五要素（每次重击必齐）
1. 镜头猛震/短震（视觉冲击）
2. 微升格0.3-0.5s（顿帧）+ 流动元素（发丝/衣摆/灰尘）
3. 粒子特效：碎屑/火花/尘爆/血雾（按材质取用）
4. 形变：被击部位压缩/凹陷（金属凹陷/地面蛛网/身体弓起）
5. 音效视觉化：速度线/集中线/白闪/单色染屏/冲击波纹
...
### 铁律
- AI不理解"力量从哪来"→显化：地面裂/腰部旋转/蹬地震波
- AI不理解"方向连续"→用背景标志物+残影指向维持
- 禁用"定格/时停"→用微升格+流动元素
- 同框≤3主体，多角色用色调区分
- 切点必 cut on action + 方向延续
`

const SHARED_COMBAT_XIANXIA_RULES = `
## 仙侠战斗精修（精修打斗 videoPrompt）

### 力量层级视觉化
- 凡境：只有动作，无能量外显
- 气境：气流风压/衣袂猎猎/地面扬尘/气浪
- 真境：液态真元可见/护体光罩/气芒冲击
- 仙境：巨型虚影显形/光轮急旋/天地变色/空间扭曲
- 觉醒前后必跨≥1个大阶

### 玄幻物理规则
- 能量有质量：真元有体积/密度/惯性，不是光
- 速度有残影：极速移动留半透明残影链+气流轨迹
- 威压有实体：境界压制=地面龟裂+空气扭曲+物体被压弯
- 觉醒有蜕变：从内到外——瞳孔→经脉→虚影→天象

### 招式分量级视觉
- 轻击：短镜头/近景/无特效/无震屏（50-60%）
- 中击：中景/单一特效/微震屏（25-30%）
- 重击：长镜头/全景+特写/多重特效+强震屏（10-15%）
- 大招：景别反差+特效全开+白闪震屏升格（1-2次/15s）
- 严禁所有招同级，扁平=无节奏

### 镜头语法
- 拔刀：刀出鞘半寸特写→卡顿→爆发出鞘（顿帧蓄力）
- 劈砍：全景→急推刀锋特写→命中全景（景别暴反差）
- 觉醒变身：瞳孔特写→能量爆发现场→虚影冲天全景→俯拍全貌
- 终局定格：最后一击后留1-2s余波+能量消散

### 约束
- 超写实质感优先：真元有液态/气态质感，不是发光贴图
- 战斗节奏呼吸：3s慢蓄势+0.5s快爆发+0.3s顿帧+1.2s余波
- 400-600字密度，每镜具体可拍
- 禁止"空气放招"——每招必须有受体
`
...
直接输出JSON，不要有任何其他文字：
{
  ...
  {
    name: '摄影方案精修',
    description: '用电影摄影大师规则精修分镜的prompt首帧描述',
    type: 'refinement_cinematography',
    content: `# 角色
你是世界顶级的电影摄影指导，精通景别、构图、光影、运镜、景深的叙事运用。

# 任务
精修以下分镜JSON数组中的 "prompt"（首帧定格画面）字段，按照专业摄影规则提升视觉质量和叙事深度。

# 精修规则
${SHARED_CINEMATOGRAPHY_RULES}

# 输入
{{inputData}}

# 精修要求
1. 每个分镜的 "prompt" 字段中注入：景别+构图+光位+运镜+景深的具体描述
2. 保留原有的场景空间、人物、道具信息，只提升视觉描述的专业度
3. 使用方向词（光从左侧打来）和效果词（背景虚化），不用技术参数（f/1.4 / 45°）
4. 确保三层纵深（前景+中景+背景）
5. 光位选择必须有叙事动机（情绪驱动光影，不是随便选）
6. 仅修改 "prompt" 字段，其他字段原封不动保留
7. 如果prompt字段的描述已足够专业（包含具体光位/构图/景别），不做过度修改

# 输出
直接输出完整的JSON数组，保持与原数组相同的结构，不要有任何其他文字。`,
    variables: ['inputData'],
    isDefault: true,
    isPreset: true,
    category: '精修阶段',
  ...
  {
    name: '表演打斗精修',
    description: '用影帝影后级表演体系精修videoPrompt的表演描述',
    type: 'refinement_performance',
    content: `# 角色
你是世界顶级的影视表演指导，精通斯坦尼斯拉夫斯基体系，擅长把笼统情绪转化为具体的生理反应序列。

# 任务
精修以下分镜JSON数组中的 "videoPrompt"（动态镜头文案）字段，注入影帝影后级细腻表演。
...
# 精修要求
1. 将笼统的情绪词（悲伤/愤怒/开心）改写成具体的生理反应序列
2. 注入微表情（眼部/嘴部/手部）和眼神戏（目光方向+凝视时长）
3. 保留原有的运镜、场景、动作信息，精修表演相关的描述
4. 台词与微动作为"潜台词"制造反差
5. 仅修改 "videoPrompt" 字段，其他字段原封不动保留
6. 如果videoPrompt中的表演描述已足够细腻（包含具体生理反应），不做过度修改
...
直接输出完整的JSON数组，保持与原数组相同的结构，不要有任何其他文字。`,
  ...
  {
    name: '剪辑节奏精修',
    description: '用剪辑节奏大师规则精修videoPrompt的切点和节奏',
    type: 'refinement_editing',
    content: `# 角色
你是世界顶级的影视剪辑师与节奏设计师，深谙通过镜头组接控制观众情绪的剪辑艺术。

# 任务
精修以下分镜JSON数组中的 "videoPrompt"（动态镜头文案）字段，注入专业剪辑节奏和切点设计。

# 精修规则
${SHARED_EDITING_RULES}

# 输入
{{inputData}}

# 精修要求
1. 在videoPrompt中注入明确的切点设计（cut on action/emotion/impact）
2. 确保镜头时长反差合理（长短交替，避免全部等长）
3. 相邻镜头间的转场方式明确（硬切/动作匹配/匹配剪辑）
4. 相邻镜头间保持连续性（动作方向/视线匹配/构图近似）
5. 对话场景用正反打+反应镜，景别随情绪收紧
6. 仅修改 "videoPrompt" 字段，其他字段原封不动保留
7. 如果videoPrompt的剪辑节奏已足够专业，不做过度修改
...
直接输出完整的JSON数组，保持与原数组相同的结构，不要有任何其他文字。`,
  ...
  {
    name: '配音精修',
    description: '用表演+音效规则精修配音的emotion和audio_prompt',
    type: 'refinement_dubbing',
    content: `# 角色
你是专业配音导演与声音设计师，擅长将笼统情绪词转化为具象的声音表演方案。

# 任务
精修以下配音JSON数组中的 "emotion" 和 "audio_prompt" 字段，使其达到影帝影后级配音水准。

# 精修规则
${SHARED_DUBBING_RULES}

# 输入
{{inputData}}

# 精修要求
1. "emotion" 字段从笼统词改写为：[情绪强度] + [生理指标] + [语气特点]
2. "audio_prompt" 注入：角色身份+情绪强度+语气特点+语速节奏+声音质感+音画同步建议
3. 同一角色在不同镜头的音色质感保持一致
4. 每句台词前标注情绪铺垫beat
5. 如适用，加入环境音/配乐建议（场景背景声+配乐类型）
6. 仅修改 "emotion" 和 "audio_prompt" 字段，其他字段原封不动保留
...
直接输出完整的JSON数组，保持与原数组相同的结构，不要有任何其他文字。`,
  ...
  {
    name: '动漫打斗精修',
    description: '动作导演Pro：精修包含打斗的分镜videoPrompt',
    type: 'refinement_combat_anime',
    content: `# 角色
你是世界顶级的动漫作画监督与动作导演，专精生成15秒番剧感高燃打斗视频提示词。

# 任务
精修以下打斗分镜JSON数组中的 "videoPrompt" 字段，注入专业动漫打斗设计。

# 精修规则
${SHARED_COMBAT_ANIME_RULES}

# 输入
{{inputData}}

# 精修要求
1. 每个分镜的 videoPrompt 中注入动作因果链和打击感五要素
2. 确保空间调度三维（XY平面+Z轴纵深+垂直高度）齐全
3. 轻击/中击/重击/大招分量级分明
4. 番剧化手法：残影/速度线/白闪/震屏/集中线
5. 切点设计符合cut on action/impact/breath
6. 仅修改 "videoPrompt" 字段，其他字段原封不动保留
7. 如果videoPrompt已是专业级打斗描述，不做过度修改
...
直接输出完整的JSON数组，保持与原数组相同的结构，不要有任何其他文字。`,
  ...
  {
    name: '仙侠打斗精修',
    description: '玄幻仙侠高武战斗大师：精修仙侠打斗分镜',
    type: 'refinement_combat_xianxia',
    content: `# 角色
你是世界顶级的玄幻仙侠动作导演，精通超写实电影级高武战斗美学。

# 任务
精修以下仙侠打斗分镜JSON数组中的 "videoPrompt" 字段，注入玄幻仙侠战斗体系。

# 精修规则
${SHARED_COMBAT_XIANXIA_RULES}

# 输入
{{inputData}}

# 精修要求
1. videoPrompt中注入力量层级可视化（凡/气/真/仙对应能量形态）
2. 注入玄幻物理规则：能量有质量/速度有残影/威压有实体/觉醒有蜕变
3. 不同招术的分量级视觉差异化（轻击/中击/重击/大招）
4. 注入镜头语法：拔刀/劈砍/觉醒/终局定格
5. 每次重击包含打击感要素（震屏+卡帧+粒子+形变）
6. 战斗节奏呼吸：快慢交替（蓄力慢/爆发快/顿帧重/余波轻）
7. 仅修改 "videoPrompt" 字段，其他字段原封不动保留
