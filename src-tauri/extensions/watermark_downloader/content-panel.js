(function () {
  ...
  chrome.runtime.onMessage.addListener((message) => {
    ...
    if (message.type === "DOUBAO_VIDS_FOUND" && Array.isArray(message.vids)) {
      fetchDoubaoVideos(message.vids);
    }
  });

  async function fetchDoubaoVideos(vids) {
    ...
    for (const vid of uniqueVids) {
      ...
      if (isHttpUrl(url)) foundItems.push({ type: "video", url });
    }
  ...
  function addItems(nextItems) {
    for (const item of nextItems) {
      if (!item || typeof item.url !== "string" || !isHttpUrl(item.url)) continue;
      items.set(item.url, {
